...
// Config for the OpenCode adapter.
type Config struct {
	Command string
	Args    []string
	Cwd     string
...
// Adapter manages the OpenCode ACP subprocess.
type Adapter struct {
...
func (a *Adapter) Initialize(ctx context.Context) (*InitResult, error) {
	resp, err := a.sub.SendRequest(ctx, adapter.Request{
		...
		Params: map[string]any{
			...
			"clientCapabilities": map[string]any{"toolExecution": true, "permissionHandling": true, "mcp": true},
		},
	})
	if err != nil {
		return nil, err
	...
	slog.Info("opencode adapter initialized", "provider", result.Provider)
	return result, nil
}

func (a *Adapter) NewSession(ctx context.Context, params SessionParams) (string, error) {
	p := map[string]any{"cwd": params.Cwd, "title": params.Title, "provider": "opencode"}
	if params.Model != "" {
...
func CheckDependencies() error {
	if _, err := exec.LookPath("opencode"); err != nil {
		return fmt.Errorf("opencode adapter: opencode not found on PATH: %w", err)
	}
	return nil
}
